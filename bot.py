...
def show_menu():
    print("\n" + "="*50)
    print("Polymarket 跟单机器人 V2.0 (修复版)")
    print("="*50)
    print("1. 检查环境并自动安装依赖")
        ...
        try:
            ...
            try:
                ...
                logs = await w3.eth.get_logs({
                    ...
                    'fromBlock': from_block,  # 使用整数，让web3处理转换
                    'toBlock': to_block,      # 使用整数
                ...
                if "hex string without 0x prefix" in error_msg:
                    logger.warning(f"🔄 十六进制格式问题，尝试修复...")
                    ...
                    try:
                        ...
                        if logs:
                            logger.info(f"修复后查询到 {len(logs)} 个事件")
        ...
        for i in range(3):
            try:
                ...
                if connected:
                    ...
                    logger.warning(f"第 {i+1} 次连接失败，重试...")
                    await asyncio.sleep(2)
            except Exception as e:
                logger.warning(f"连接测试失败: {e}")
                await asyncio.sleep(2)
        else:
            logger.error("❌ 无法连接到 RPC，请检查网络和RPC地址")
            return
        
        # 获取当前区块
        try:
            current_block = await w3.eth.block_number
            start_block = max(current_block - 20, 0)
...
def main():
    print("\n" + "="*60)
    print(" "*15 + "Polymarket 跟单机器人 V2.0 (修复版)")
    print("="*60)
    print("说明：")
